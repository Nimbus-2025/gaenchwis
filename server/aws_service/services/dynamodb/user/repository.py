...
class UserRepository(BaseRepository):
    ...
    def get_user_schedules(self, user_id: str) -> List[Dict]:
        """사용자의 일정 목록 조회"""
        try:
            return self.get_by_keys(f"USER#{user_id}", "SCHEDULE#")
        except Exception as e:
            raise OperationException(f"일정 조회 중 오류 발생: {str(e)}")

    def get_user_bookmarks(self, user_id: str) -> List[Dict]:
        """사용자의 북마크 목록 조회"""
        try:
            return self.get_by_keys(f"USER#{user_id}", "BOOKMARK#")
        except Exception as e:
            raise OperationException(f"북마크 조회 중 오류 발생: {str(e)}")

    def get_user_applies(self, user_id: str) -> List[Dict]:
        """사용자의 지원 내역 조회"""
        try:
            return self.get_by_keys(f"USER#{user_id}", "APPLY#")
        except Exception as e:
            raise OperationException(f"지원 내역 조회 중 오류 발생: {str(e)}")

    def get_user_interest_companies(self, user_id: str) -> List[Dict]:
        """사용자의 관심 기업 목록 조회"""
        try:
            return self.get_by_keys(f"USER#{user_id}", "COMPANY#")
        except Exception as e:
            raise OperationException(f"관심 기업 조회 중 오류 발생: {str(e)}")